...
import tshuttle.helpers


@patch('tshuttle.helpers.logprefix', new='prefix: ')
@patch('tshuttle.helpers.sys.stdout')
@patch('tshuttle.helpers.sys.stderr')
...
@patch('tshuttle.helpers.logprefix', new='prefix: ')
@patch('tshuttle.helpers.verbose', new=1)
@patch('tshuttle.helpers.sys.stdout')
@patch('tshuttle.helpers.sys.stderr')
def test_debug1(mock_stderr, mock_stdout):
    tshuttle.helpers.debug1("message")
    assert mock_stdout.mock_calls == [
        call.flush(),
    ]
    assert mock_stderr.mock_calls == [
...
@patch('tshuttle.helpers.logprefix', new='prefix: ')
@patch('tshuttle.helpers.verbose', new=0)
@patch('tshuttle.helpers.sys.stdout')
@patch('tshuttle.helpers.sys.stderr')
...
@patch('tshuttle.helpers.logprefix', new='prefix: ')
@patch('tshuttle.helpers.verbose', new=2)
@patch('tshuttle.helpers.sys.stdout')
@patch('tshuttle.helpers.sys.stderr')
def test_debug2(mock_stderr, mock_stdout):
    tshuttle.helpers.debug2("message")
    assert mock_stdout.mock_calls == [
        call.flush(),
    ]
    assert mock_stderr.mock_calls == [
...
@patch('tshuttle.helpers.open', create=True)
...
""")
    ...
    ns = tshuttle.helpers.resolvconf_nameservers(False)
    assert ns == [
...
nameserver 2404:6800:4004:80c::4
""")
    ns = tshuttle.helpers.resolvconf_random_nameserver(False)
    assert ns in [
        (AF_INET, u'192.168.1.1'), (AF_INET, u'192.168.2.1'),
        (AF_INET, u'192.168.3.1'), (AF_INET, u'192.168.4.1'),
        (AF_INET6, u'2404:6800:4004:80c::1'),
...
@patch('tshuttle.helpers.socket.socket.bind')
def test_islocal(mock_bind):
    ...
    assert tshuttle.helpers.islocal("127.0.0.1", AF_INET)
    assert not tshuttle.helpers.islocal("192.0.2.1", AF_INET)
    assert tshuttle.helpers.islocal("::1", AF_INET6)
    assert not tshuttle.helpers.islocal("2001:db8::1", AF_INET6)


def test_family_ip_tuple():
    assert tshuttle.helpers.family_ip_tuple("127.0.0.1") \
        == (AF_INET, "127.0.0.1")
    assert tshuttle.helpers.family_ip_tuple("192.168.2.6") \
        == (AF_INET, "192.168.2.6")
    assert tshuttle.helpers.family_ip_tuple("::1") \
        == (AF_INET6, "::1")
    assert tshuttle.helpers.family_ip_tuple("2404:6800:4004:80c::1") \
